...
public class ExamenPilaMapa {
    ...
    public static boolean esBalanceado(String s) {
        ...
        return pila.estaVacia(); // true si todos los '(' se cerraron
    }

    
    public static boolean actualizarCalificacion(Map<Integer, Integer> califPorId, int id, int nuevo) {
        if (nuevo < 0 || nuevo > 100) return false; // validación de rango
        if (!califPorId.containsKey(id)) return false; // debe existir el ID
